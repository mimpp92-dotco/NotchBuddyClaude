...
/// baseScale 파라미터로 MascotSize에 따른 크기를 유지하면서 애니메이션한다.
enum StateAnimations {

    /// 상태별 반복 애니메이션을 반환한다.
    static func animation(for state: MascotState, baseScale: CGFloat = 1.0) -> SKAction {
        switch state {
        case .idle:
        ...
        case .needsInput:
            ...
            let scaleUp = SKAction.scale(to: baseScale * 1.08, duration: 0.15)
            let scaleDown = SKAction.scale(to: baseScale, duration: 0.15)
    ...
    static func transitionAction(to state: MascotState, baseScale: CGFloat = 1.0) -> SKAction {
        switch state {
        case .needsInput:
            let scaleUp = SKAction.scale(to: baseScale * 1.3, duration: 0.1)
            let scaleDown = SKAction.scale(to: baseScale, duration: 0.15)
            scaleDown.timingMode = .easeOut
            return SKAction.sequence([scaleUp, scaleDown])

        case .done:
            let scaleUp = SKAction.scale(to: baseScale * 1.3, duration: 0.12)
            let scaleDown = SKAction.scale(to: baseScale, duration: 0.1)
            return SKAction.sequence([scaleUp, scaleDown])

        case .working:
            let scaleUp = SKAction.scale(to: baseScale * 1.1, duration: 0.1)
            let scaleDown = SKAction.scale(to: baseScale, duration: 0.1)
            return SKAction.sequence([scaleUp, scaleDown])

        case .error:
            let scaleUp = SKAction.scale(to: baseScale * 1.4, duration: 0.08)
            let scaleDown = SKAction.scale(to: baseScale, duration: 0.12)
            scaleDown.timingMode = .easeOut
            return SKAction.sequence([scaleUp, scaleDown])
