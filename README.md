# NotchBuddy Claude

MacBook 노치에 사는 Claude Code 동반자 앱.

Claude Code로 작업할 때, 노치 옆에 작은 마스코트가 나타나서 현재 세션 상태를 실시간으로 보여줍니다.
마스코트를 데스크탑 아무 곳에 배치할 수도 있습니다.

## Features

- **실시간 상태 반영** — idle, working, needsInput, done, error, playing 6가지 상태
- **애니메이션** — 상태별 고유 모션, 눈 깜빡임, 말풍선, 파티클 이펙트
- **자유 배치** — 롱프레스(0.5s) 드래그로 노치 또는 데스크탑 아무 곳에 배치
- **마스코트 선택** — Claude, Rabbit, White Cat, Black Cat 4종
- **마스코트 크기** — 데스크탑 모드에서 S/M/L 크기 선택 가능
- **메뉴바 팝오버** — 메뉴바 아이콘 또는 마스코트 클릭으로 세션 목록 + 설정 패널
- **세션 앱 감지** — Terminal, VS Code, Warp, Claude Desktop 등 자동 판별 + 앱 아이콘 표시
- **스마트 포커스** — 세션 클릭 시 해당 앱으로 즉시 포커스
- **멀티 세션 추적** — 모든 활성 세션 한눈에 확인
- **4개 언어** — 한국어, English, 日本語, 中文
- **오프닝 연출** — 앱 시작 시 Claude 오렌지 "Notch Buddy" + 마스코트 등장
- **랜덤 반응** — 대기 중 하품, 깜빡임, 기지개, 놀람 등

## Installation

터미널에 아래 한 줄을 붙여넣으세요:

```bash
curl -sL https://raw.githubusercontent.com/mimpp92-dotco/NotchBuddyClaude/main/install.sh | bash
```

자동으로 다운로드 → 설치 → 실행됩니다.

### 수동 설치

1. [Releases](https://github.com/mimpp92-dotco/NotchBuddyClaude/releases)에서 `NotchBuddy-v*.zip` 다운로드
2. 압축 풀고 `NotchBuddy.app`을 `/Applications`로 이동
3. 터미널에서 `xattr -cr /Applications/NotchBuddy.app` 실행
4. 앱 실행

### 제거

```bash
rm -rf /Applications/NotchBuddy.app
```

## Requirements

- macOS 12 이상 (노치가 있는 MacBook 권장)
- [Claude Code CLI](https://docs.anthropic.com/en/docs/claude-code)

## How It Works

1. 앱 실행 시 Claude Code hooks를 자동 설정합니다
2. Claude Code 세션이 시작되면 HTTP hook 이벤트를 수신합니다
3. 마스코트가 실시간으로 상태를 반영합니다
4. 메뉴바 팝오버에서 모든 세션의 상태를 확인하고 앱 간 전환할 수 있습니다

| 상태 | 마스코트 동작 |
|------|-------------|
| Idle | 호흡 애니메이션, 좌우 기울임 |
| Working | 바운스 + 미세 회전 |
| Needs Input | 흔들기 + 스케일 펄스 |
| Done | 점프 + 기울임 + 초록 틴트 |
| Error | 주기적 흔들림 + 빨간 틴트 |
| Playing | 좌우 이동 + 점프 |

## Usage

- **메뉴바 아이콘 클릭** — 팝오버 패널 (세션 목록 + 마스코트 선택 + 설정)
- **마스코트 클릭** — 동일한 팝오버 패널
- **롱프레스(0.5s) + 드래그** — 마스코트를 원하는 위치로 이동
- **노치 영역에 놓기** — 노치 모드로 복귀 (S 크기 고정)
- **데스크탑에 놓기** — 데스크탑 모드 (S/M/L 크기 선택 가능)
- **세션 클릭** — 해당 앱으로 즉시 포커스 (Terminal, VS Code, Warp, Claude Desktop 등)

### 팝오버 메뉴

| 버튼 | 기능 |
|------|------|
| 마스코트 | 4종 마스코트 선택 (Claude, Rabbit, White Cat, Black Cat) |
| 설정 | 마스코트 크기 (S/M/L) + 언어 선택 (ko/en/ja/zh) |
| 종료 | 앱 종료 |

## Build from Source

```bash
git clone https://github.com/mimpp92-dotco/NotchBuddyClaude.git
cd NotchBuddyClaude
swift build -c release
```

## License

MIT
